...
public class DrListByHospitalActivity extends AppCompatActivity implements  ApiListener.drSearchListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_dr_list_by_hospital);
        ButterKnife.bind(this);
        sessionManager=new SessionManager(this);
        key=sessionManager.getToken();
        USER_ID=sessionManager.getUserId();
        int LENGTH=15;
        if (DataStore.hospitalID.getName().length()>=LENGTH){
            tv_name.setText(DataStore.hospitalID.getName().substring(0,LENGTH)+"......");
        }else {
            tv_name.setText(DataStore.hospitalID.getName());

        }

        Api.getInstance().searchByHospitalDoctor(key, ""+DataStore.hospitalID.getId(),this);
    }
    @Override
